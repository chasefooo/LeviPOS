...
exports.handler = async (event) => {
    ...
    try {
        if (method === 'GET') {
            ...
            await pool.query(
                `INSERT INTO Discounts (${columns.join(',')}) VALUES (${placeholders})`,
                values
            );
            ...
            let data = JSON.parse(event.body);
            if (typeof data === 'string') data = JSON.parse(data);
            const idToUpdate = data.DiscountID || id;
            const columns = Object.keys(data).filter(col => col !== 'DiscountID');
            const assignments = columns.map(col => `${col} = ?`).join(',');
            const values = columns.map(col => data[col]);
            values.push(idToUpdate);
            await pool.query(
                `UPDATE Discounts SET ${assignments} WHERE DiscountID = ?`,
                values
            );
